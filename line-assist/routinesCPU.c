	...
	int height, int width)
{
	...
	//esto gpu bien currado
	//un kernel
	for(i=2; i<height-2; i++)
		for(j=2; j<width-2; j++)
		{
			// Noise reduction (el blur)
			NR[i*width+j] =
	...
	for(i=2; i<height-2; i++)
		for(j=2; j<width-2; j++)
		{
			// Intensity gradient of the image intensidad de la imagen en el eje de las x y en el eje de las y
			Gx[i*width+j] = 
			...
			//los valores del kernel estan rotados
			//tener en cuenta los accesos a memoria en el eje de las 'y' porque mucho coste
			Gy[i*width+j] = 
